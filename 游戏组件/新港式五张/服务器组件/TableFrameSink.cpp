...
bool CTableFrameSink::OnEventGameStart()
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		if ( pIServerUserItem != NULL)
		{
			...
			CopyMemory(CheatCard.cbHandCardData[i], m_cbHandCardData[i], sizeof(CheatCard.cbHandCardData[i]));
		}
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		if (pIServerUserItem!=NULL)
		{
			...
			{
				m_pITableFrame->SendTableData(i,SUB_S_CHEAT_CARD,&CheatCard,sizeof(CheatCard));
			}
		}
	}
...
bool CTableFrameSink::OnGetUserBetInfo(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	printf("处理游戏下注信息:CTableFrameSink::OnGetUserBetInfo\n");
